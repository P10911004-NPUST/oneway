transform_options <- c("art", "sq", "sqrt", "log", "log2", "log10", "recip")

transform <- function(data, formula, method = "art", simplify = FALSE, misc = FALSE)
{
    ...
    if (method == "art")
        ret <- func(data, formula, simplify, misc)
    else
        ret <- func(data)
